...
public class AdminServlet extends BaseServlet {
	...
	private void handleReportersStat( final HttpServletRequest request, final HttpServletResponse response ) throws IOException, ServletException {
		...
		try {
			...
			statement = connection.createStatement();
			resultSet = statement.executeQuery( "SELECT key.id, key.value, key.revocated, person.name, COUNT(report.id), COUNT(DISTINCT hacker.id), COUNT(CASE WHEN report.revocated=TRUE OR key.revocated=TRUE OR hacker.guarded=TRUE THEN 1 END), COUNT(report.id)/(1.000+CAST(CAST(MAX(report.version) AS DATE)-CAST(MIN(report.version) AS DATE) AS bigint)), MIN(report.version), MAX(report.version), 1 + date(MAX(report.version)) - date(MIN(report.version)) FROM report JOIN key on report.key=key.id JOIN person on key.person=person.id JOIN hacker on report.hacker=hacker.id GROUP BY key.id, key.value, key.revocated, person.name ORDER BY COUNT(report.id) DESC;" );
			
			int rowCounter = 0;
			while ( resultSet.next() ) {
				outputWriter.println( "<tr><td align=right>" + (++rowCounter)
						+ "<td align=right>" + ( resultSet.getBoolean( 3 ) ? "(revocated) " : "" ) + "<a href='admin?" +  REQUEST_PARAM_PAGE_NAME + '=' + Page.LAST_REPORTS.name() + '&' + REQUEST_PARAM_KEY_ID + "=" + resultSet.getInt( 1 ) + "'>" + resultSet.getInt( 1 ) + "</a>" + ( fullAdmin ? ' ' + getHackerRecordsByKeyLink( resultSet.getString( 2 ), "", "keyreportsform" ) : "" )
						+ "<td>" + encodeHtmlString( resultSet.getString( 4 ) )
						+ "<td align=right>" + resultSet.getInt( 5 ) + "<td align=right>" + resultSet.getInt( 6 )
						+ "<td align=right>" + resultSet.getInt( 7 )
						+ "<td align=right>" + String.format( "%.3f", resultSet.getFloat( 8 ) )
						+ "<td align=center>" + formatDays( resultSet.getInt( 11 ) )
						+ "<td>" + TIME_FORMAT.format( resultSet.getTimestamp( 9 ) )
						+ "<td>" + TIME_FORMAT.format( resultSet.getTimestamp( 10 ) ) );
			}
			resultSet.close();
			statement.close();
